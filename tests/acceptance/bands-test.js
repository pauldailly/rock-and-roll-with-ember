...
module('Acceptance | Bands', function (hooks) {
  ...
  test('Sort songs in various ways', async function (assert) {
    ...
    server.create('song', { title: 'Elephants', rating: 5, band });
    server.create('song', { title: 'New Fang', rating: 4, band });
    server.create('song', {
      title: 'Mind Eraser, No Chaser', rating:
        4, band
    });
    server.create('song', {
    ...
    assert.dom('[data-test-rr=song-list-item]:last-child').hasText('New Fang', 'The last song is the one that is rated lowest');
  });
